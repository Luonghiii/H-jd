...
const MAX_WORD_IMAGE_DIMENSION = 512; // Max dimension for word illustrations
...
export const resizeWordImageAsDataUrl = (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
        const reader = new FileReader();
        reader.readAsDataURL(file);
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                if (width > MAX_WORD_IMAGE_DIMENSION || height > MAX_WORD_IMAGE_DIMENSION) {
                    if (width > height) {
                        height = Math.round(height * (MAX_WORD_IMAGE_DIMENSION / width));
                        width = MAX_WORD_IMAGE_DIMENSION;
                    } else {
                        width = Math.round(width * (MAX_WORD_IMAGE_DIMENSION / height));
                        height = MAX_WORD_IMAGE_DIMENSION;
                    }
                }
                ...
                if (dataUrl.length > 300 * 1024) { // 300KB is a reasonable limit for word images
                     return reject(new Error('Ảnh quá lớn (sau khi nén > 300KB). Vui lòng chọn ảnh nhỏ hơn.'));
                }
