...
import { Trash2, Search, ImageIcon, Sparkles, Star, ChevronDown } from 'lucide-react';
...
const WordList: React.FC = () => {
  const { words, lastDeletedWord, undoDelete, updateWordImage } = useVocabulary();
  const { targetLanguage } = useSettings();
  ...
  const groupedWords = useMemo(() => {
    ...
    const sortedWords = [...words].sort((a, b) => (b.isStarred ? 1 : 0) - (a.isStarred ? 1 : 0) || b.createdAt - a.createdAt);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="max-h-[50vh] overflow-y-auto pr-2">
        {searchTerm ? (
        ...
        ) : groupedWords && groupedWords.length > 0 ? (
          <div className="space-y-4">
            {groupedWords.map(([theme, themeWords]) => (
              <details key={theme} className="group">
          ...
          <p className="text-center text-gray-400 py-8">
            Danh sách từ của bạn trống. Hãy thêm từ để bắt đầu!
          </p>
        )}
      </div>
