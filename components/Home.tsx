...
const TimeIsWidgetClock: React.FC = () => {
  ...
  return (
    <div className="text-center my-6">
      <a href="https://time.is/Vietnam" id="time_is_link" rel="nofollow" className="text-xl text-indigo-600/80 no-underline hover:underline">
        Thời gian ở Việt Nam:
      </a>
      {/* The script will populate this div */}
      <div id="Vietnam_z418" className="text-4xl font-bold text-slate-800 mt-1"></div>
    </div>
  );
};
...
const QuickReview: React.FC = () => {
    ...
    if (!isSessionActive || sessionWords.length === 0) {
        return (
            <div className="bg-white/50 border border-white/30 rounded-2xl p-6 flex flex-col items-center justify-center text-center h-full neu-light">
                <BrainCircuit className="w-8 h-8 text-emerald-500 mb-2" />
                <h3 className="font-bold text-slate-800">{t('home.quick_review')}</h3>
    ...
    return (
        <div className="bg-white/50 border border-white/30 rounded-2xl p-4 flex flex-col h-full neu-light">
            <h3 className="font-bold text-slate-800 mb-2 text-center">{t('home.quick_review')} ({currentIndex + 1}/{sessionWords.length})</h3>
                ...
                <div className="[perspective:1000px]" onClick={() => setIsFlipped(!isFlipped)}>
                    ...
                    >
                        <div 
                            ...
                            className="absolute w-full h-full flex items-center justify-center p-2 bg-slate-100/70 rounded-lg"
                            style={{
                                backfaceVisibility: 'hidden',
                                WebkitBackfaceVisibility: 'hidden', // For Safari/iOS
                            }}
                        >
                            <p className="text-xl font-bold text-slate-800 text-center">{currentWord.word}</p>
                        </div>
                        <div 
            ...
            {isFlipped ? (
                 <div className="grid grid-cols-3 gap-2 mt-3 animate-fade-in">
                    <button onClick={() => handlePerformance('hard')} className="py-2 bg-red-500/20 hover:bg-red-500/40 text-red-600 text-sm font-semibold rounded-lg">{t('home.quick_review_hard')}</button>
                    <button onClick={() => handlePerformance('good')} className="py-2 bg-amber-500/20 hover:bg-amber-500/40 text-amber-600 text-sm font-semibold rounded-lg">{t('home.quick_review_good')}</button>
                    <button onClick={() => handlePerformance('easy')} className="py-2 bg-green-500/20 hover:bg-green-500/40 text-green-600 text-sm font-semibold rounded-lg">{t('home.quick_review_easy')}</button>
                 </div>
            ) : (
    ...
    if (!word) {
         return (
            <div className="bg-white/50 border border-white/30 rounded-2xl p-6 flex flex-col items-center justify-center text-center h-full neu-light">
                ...
                <h3 className="font-bold text-slate-800">{t('home.word_of_the_day')}</h3>
                <p className="text-sm text-gray-600 mt-1">{t('home.add_words_prompt')}</p>
            </div>
        );
    }

    return (
        <div className="bg-white/50 border border-white/30 rounded-2xl p-4 flex flex-col h-full cursor-pointer neu-light" onClick={() => openInspector(word)}>
            <h3 className="font-bold text-slate-800 mb-2 text-center">{t('home.word_of_the_day')}</h3>
            <div className="flex-grow flex flex-col items-center justify-center text-center">
                 {word.imageUrl && <img src={word.imageUrl} alt={word.word} className="w-full h-24 object-contain rounded-md mb-3" />}
                <p className="text-2xl font-bold text-cyan-600">{word.word}</p>
                <p className="text-gray-600">{word.translation[uiLanguage]}</p>
            </div>
    ...
    <div className="space-y-8 animate-fade-in text-slate-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white/50 p-5 rounded-2xl flex items-center gap-4 border border-white/30 neu-light">
            ...
            <div>
                {isWordsLoading ? (
                    <div className="h-7 w-16 bg-slate-200 rounded-md animate-pulse"></div>
                ) : (
                    <div className="text-3xl font-bold">{words.length}</div>
                )}
                <div className="text-sm text-gray-600">{t('home.saved_words')}</div>
            </div>
        </div>
        <div className="bg-white/50 p-5 rounded-2xl flex items-center gap-4 border border-white/30 neu-light">
            ...
            <div>
                {isSettingsLoading ? (
                    <div className="h-7 w-8 bg-slate-200 rounded-md animate-pulse"></div>
                ) : (
                    <div className="text-3xl font-bold">{stats.currentStreak}</div>
                )}
                <div className="text-sm text-gray-600">{t('home.current_streak')}</div>
            </div>
        </div>
        <div className="bg-white/50 p-5 rounded-2xl flex items-center gap-4 border border-white/30 neu-light">
            ...
            <div>
                ...
                <div className="text-sm text-gray-600">{t('home.longest_streak')}</div>
            </div>
        </div>
      </div>
      ...
      <div>
        <h2 className="text-2xl font-bold text-slate-800 mb-4">{t('home.learning_modes')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {featureCards.map((card) => (
            <div
              key={card.view}
              onClick={() => setCurrentView(card.view)}
              className="group bg-white/50 border border-white/30 rounded-2xl p-6 flex flex-col transition-all duration-300 cursor-pointer neu-button-light"
            >
