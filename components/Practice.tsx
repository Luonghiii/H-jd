    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Luyện tập Viết</h2>
          <p className="text-slate-500 dark:text-gray-400 mt-1">Dịch từ để củng cố kiến thức.</p>
        </div>
        <div>
        ...
        <div>
          <h3 className="font-semibold text-slate-800 dark:text-white mb-2">2. Chọn số từ</h3>
          <div className="flex justify-center gap-2">
            {[5, 10, 20].map(n => (
              <button key={n} onClick={() => setNumWords(n)} className={`px-4 py-2 text-sm rounded-xl transition-colors ${numWords === n ? 'bg-indigo-600 text-white' : 'bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600'}`}>{n} từ</button>
            ))}
            <button onClick={() => setNumWords(wordsForPractice.length)} className={`px-4 py-2 text-sm rounded-xl transition-colors ${numWords === wordsForPractice.length ? 'bg-indigo-600 text-white' : 'bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600'}`}>Tất cả ({wordsForPractice.length})</button>
          </div>
        </div>
  ...
  if (view === 'results') {
    ...
    return (
      <div className="space-y-6 text-center animate-fade-in">
        ...
        <p className="text-xl text-slate-600 dark:text-gray-300">Bạn đã trả lời đúng {correctCount} trên {answers.length} từ!</p>
        <div className="text-left space-y-3 pt-4">
            <h3 className="font-semibold text-slate-900 dark:text-white">Xem lại các câu trả lời:</h3>
                ...
                {answers.map(({ word, userAnswer, isCorrect }, index) => (
                    <div key={index} className={`p-3 rounded-xl ${isCorrect ? 'bg-green-500/10' : 'bg-red-500/10'}`}>
                        ...
                        <p className="text-sm">{isCorrect ? 
                            <span className="text-green-600 dark:text-green-400">Chính xác: {userAnswer}</span> : 
                            <>
                                <span className="text-red-500 dark:text-red-400 line-through">Bạn trả lời: {userAnswer}</span><br />
                                <span className="text-green-600 dark:text-green-400">Đáp án đúng: {word.translation[targetLanguage]}</span>
                            </>
                        }</p>
                    </div>
                ))}
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <button onClick={() => setView('setup')} className="flex-1 px-4 py-3 bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-white font-semibold rounded-xl">Luyện tập lại</button>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Câu hỏi {currentWordIndex + 1} / {practiceWords.length}</h2>
          <button onClick={() => setView('setup')} className="text-sm text-indigo-500 dark:text-indigo-400 hover:underline">Thoát</button>
        </div>
      ...
      <div className="text-center p-8 bg-slate-100 dark:bg-slate-800/50 rounded-2xl">
        <p className="text-3xl font-bold text-slate-900 dark:text-white">{currentWord.word}</p>
        <p className="text-slate-500 dark:text-gray-400 mt-1">Dịch sang {targetLanguage === 'vietnamese' ? 'Tiếng Việt' : 'Tiếng Anh'}</p>
      </div>
      <form onSubmit={handleSubmitAnswer} className="space-y-4">
        <input 
          ...
          className="w-full px-4 py-3 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-lg"
