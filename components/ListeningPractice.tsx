...
import { ArrowLeft, RefreshCw, Volume2, Check, X, Loader2, ChevronDown } from 'lucide-react';
...
const ListeningPractice: React.FC<ListeningPracticeProps> = ({ onBack }) => {
    ...
    const wordsForGame = themeFilteredWords; // Alias for clarity
                ...
                <div className="flex items-center justify-between">
                 ...
                 <details className="group bg-slate-800/50 border border-slate-700 rounded-2xl">
                    <summary className="list-none p-3 cursor-pointer flex justify-between items-center">
                        <h3 className="font-semibold text-white">Chọn chủ đề ({selectedThemes.has('all') ? 'Tất cả' : `${selectedThemes.size} đã chọn`})</h3>
                        <ChevronDown className="w-5 h-5 text-gray-400 group-open:rotate-180"/>
                    </summary>
                    <div className="p-3 border-t border-slate-600">
                        <div className="flex flex-wrap gap-2">
                            <button onClick={() => handleThemeToggle('all')} className={`px-3 py-1 text-sm rounded-full ${selectedThemes.has('all') ? 'bg-indigo-600' : 'bg-slate-700'}`}>Tất cả ({words.length})</button>
                            {availableThemes.map(theme => <button key={theme} onClick={() => handleThemeToggle(theme)} className={`px-3 py-1 text-sm rounded-full ${selectedThemes.has(theme) ? 'bg-indigo-600' : 'bg-slate-700'}`}>{themeTranslationMap[theme] || theme} ({words.filter(w => w.theme === theme).length})</button>)}
                        </div>
                    </div>
                </details>
                <div>
                    <h3 className="font-semibold text-white mb-2">Số từ</h3>
