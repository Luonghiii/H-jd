  ...
  if (view === 'setup') {
    return (
      <div className="space-y-6 animate-fade-in">
        ...
        <div>
          <h3 className="font-semibold text-white mb-2 flex items-center gap-2"><Sparkles className="w-5 h-5 text-amber-400"/> Bộ thẻ gợi ý</h3>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => handleSmartSet('review')} className="px-3 py-1 text-xs bg-slate-700 hover:bg-slate-600 rounded-lg">10 từ cần ôn</button>
            <button onClick={() => handleSmartSet('hardest')} className="px-3 py-1 text-xs bg-slate-700 hover:bg-slate-600 rounded-lg">10 từ khó nhất</button>
            <button onClick={() => handleSmartSet('oldest')} className="px-3 py-1 text-xs bg-slate-700 hover:bg-slate-600 rounded-lg">10 từ cũ nhất</button>
          </div>
        </div>
        ...
        <details className="group bg-slate-800/50 border border-slate-700 rounded-2xl">
            <summary className="list-none p-3 cursor-pointer flex justify-between items-center">
                <h3 className="font-semibold text-white">Tùy chỉnh</h3>
                <ChevronDown className="w-5 h-5 text-gray-400 transition-transform group-open:rotate-180" />
            </summary>
            <div className="p-3 border-t border-slate-600 space-y-4">
               <div>
                 <h3 className="font-semibold text-white mb-2">1. Chọn chủ đề <span className="text-gray-400 font-normal text-sm">({selectedThemes.has('all') ? 'Tất cả' : `${selectedThemes.size} đã chọn`})</span></h3>
                  <div className="flex flex-wrap gap-2">
